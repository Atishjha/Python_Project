...
pane1 = Label(root, image=new)
pane1.place(x=0, y=520)
# Dates
...
if int((dt.strftime('%I'))) >= 8 & int((st.strftime('%I'))) <=5:
    img = ImageTk.PhotoImage(Image.open('moon.png'))
    pane1 = Label(root, image=img)
    pane1.place(x=210, y=200)
else:
...
def city_name():
    api_key='c4230741e6ebd0ae930bec54e42aeeb0'
    api_request = request.get("https://api.openweathermap.org/data/2.5/weather?q="+ city_entry.get() + "&units=metric&appid=" + api_key)
